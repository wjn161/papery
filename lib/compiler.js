 ...
 * @param  {Object} opts 配置对象
 */
exports.compileListPage = function (root, opts) {
    if (root.slice(-1) !== '/') {
        root += '/';
    }
    var tpl = loadTemplate(root, 'list');

    if (typeof opts !== 'object') {
        opts = parser.parseYaml(root);
    }
    opts.filename = root + 'templates/list.ejs';

    var html = ejs.render(tpl, opts);

    fs.writeFileSync(root + '/list.html', html);

    logger.info('Build list.html completed!');
};
...
exports.compile = function (root) {
    ...
    this.compileListPage(root, opts);
